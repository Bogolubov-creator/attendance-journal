  ...
  const render = () => {
    ...
    root.querySelector("#daily-results").innerHTML =
      rows
        .map(
          (s) =>
            `<details class="daily-person ${s.absenceAlert ? "has-alert" : ""}"><summary><strong>${esc(s.name)}</strong>${s.absenceAlert ? `<span class="pill red">Тревога · ${s.absenceDays} уч. дней без явки</span>` : ""}<span class="daily-result ${s.status}">${labels[s.status]}${s.status === "absent" ? ` · ${s.absenceDays} уч. дн.` : ""}</span><span class="muted">Последний раз: ${s.lastVisit || "нет отметок"} · дней за период: ${s.daysPresent}</span></summary><div class="daily-history"><p>Ответы за выбранный период</p>${s.history.length ? s.history.map((r) => `<div class="daily-row"><span>${esc(r.date)} · ${esc(r.teacher)}${r.course ? " · " + esc(r.course) : ""}${r.source === "lesson" ? " · из журнала занятий" : ""}</span><strong>${r.status === "present" ? "Присутствовал(а)" : "Отсутствовал(а)"}</strong></div>`).join("") : '<p class="muted">Ответов нет</p>'}</div></details>`,
        )

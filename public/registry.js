...
export async function registryView({ api, esc, toast }) {
  ...
  root.innerHTML = `<div class="page-heading"><div><div class="eyebrow">Руководство</div><h1>Реестр преподавателей</h1><p>Реестр хранится в базе журнала. Студенты и их связи с преподавателями правятся в карточке студента на вкладке «Студенты».</p></div></div><form class="daily-toolbar panel daily-panel" id="teacher-add"><label>ФИО нового преподавателя<input name="name" required minlength="3" maxlength="150" autocomplete="off"></label><button class="btn primary">+ Добавить преподавателя</button></form><section class="panel daily-panel"><div class="daily-toolbar"><input type="search" id="teacher-search" placeholder="Найти преподавателя" aria-label="Найти преподавателя"><span class="muted">Всего: ${teachers.length}</span></div><div id="teacher-rows"></div></section>`;
  const render = () => {
    const q = root

...
export async function registryView({
  ...
  // Отдельная колонка на широком экране; на телефоне строка встаёт под имя.
  const accessLine = (p) =>
    `<span class="access-cell"><span class="${p.access.state === "none" ? "pill red" : "muted"}">Доступ: ${esc(accessLabel(p.access))}</span>${p.access.login ? `<br><span class="muted">${esc(p.access.login)}</span>` : ""}</span>`;

...
function loginView() {
  ...
  const loginError = {
    ...
    closed: "Личный кабинет закрыт: обучение завершено",
  }[new URLSearchParams(location.search).get("error")];
  if (loginError) {
    $("#login-error").textContent = loginError;
...
const changeLabels = {
  ...
  "student.delete": "Удалён студент",
  "teacher.add": "Добавлен преподаватель",
  "teacher.rename": "Изменено ФИО преподавателя",
...
function adminView() {
    ...
    `<section class="panel mini-panel"><h2>${title} <span class="muted">${rows.length}</span></h2>${
      rows.length
        ? rows
            ...
            .map(
              (s) =>
                `<div class="record"><div><button class="student-link" data-profile="${s.id}">${esc(s.name)}</button><small>${esc(s.manager?.name || "Менеджер не назначен")}</small></div><span class="pill red">${type === "absence" ? s.days + " уч. дней" : s.procedureOverdue + " требований"}</span></div>`,
            )
            .join("")
  ...
  <div class="filter-tabs">${[
    ...
    ["debt", "Не выполнены требования"],
    ["combined", "Пропуски и требования"],
    ["review", "Требования на проверке"],
    ["unknown", "Нет данных о требованиях"],
...
async function profile(id) {
  const data = await api("/api/admin/students/" + id),
    s = data.student,
    d = $("#profile"),
    teachers = data.canEdit ? await api("/api/admin/teachers") : [];
